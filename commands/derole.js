module.exports.run = async (bot, message, args) => {
    ...
    if (!args[0])
      return message.channel.send("**Usage : ?derole bots/humans/@user @role**");
    ...
    if (args[0] == "bots") {
      ...
      await message.channel.send(
        `Removed Role \`${role.name}\` To All Bots ! (${botsofserver.size}))`
      );
      ...
      await message.channel.send(
        `Removed Role \`${role.name}\` To All Humans ! (${manofserver.size}))`
      );
    }
  };
